...
void AutoOutposeNode::GetTargerAngle(const cv::Point3f armor_center){
    ...
    double yaw = asin(armor_center.x / sqrt(armor_center.x*armor_center.x+armor_center.z*armor_center.z)) * 180 / CV_PI ;  
...
cv::Point3f xyz_transformation(cv::Point3f input_point){        //坐标系转换  相机坐标系-->绝对坐标系  (shang z  qian y  you x)
    ...
    for(int i=0;i<4;i++){
        for(int j=0;j<4;j++){
            for(int g=0;g<4;g++){
                matrix_target[0][i] += matrix_A[0][i]*matrix_T[g][j];
            }
        }
    }
...
void AutoOutposeNode::OutposeCallback(const auto_aim_interfaces::msg::Armors outpose_info)
{
    ...
    for(auto outpose_armor : outpose_info.armors)
    {
        ...
        std::cout<<armor_point_tf<<std::endl;

        GetTargerAngle(armor_point_camera);

        x_next = get_next_x(armor_point_camera.x);

        data.yaw = yaw_ + gun_yaw_;
        data.pitch = pitch_ + gun_pitch_;
