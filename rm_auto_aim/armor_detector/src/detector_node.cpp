...
: Node("armor_detector", options)
{
  ...
  outpose_sub_ = this->create_subscription<std_msgs::msg::UInt32>(
    "/outpose_bool", rclcpp::SensorDataQoS(),
    [this](std_msgs::msg::UInt32::ConstSharedPtr msg_temp_){
      isShow_ = msg_temp_->data;
  });  
  ...
  if (pnp_solver_ != nullptr) {
    ...
    for (const auto & armor : armors) {
      ...
      if (success) {
        ...
        ROI_OUTPOST_YMIN = armor.left_light.top.y - 50;
        ...
        ROI_OUTPOST_HEIGHT = (((armor.left_light.bottom.y-armor.left_light.top.y)+(armor.right_light.bottom.y-armor.right_light.top.y))/2) + 100;
  ...
  const sensor_msgs::msg::Image::ConstSharedPtr & img_msg)
{
  ...
  if (debug_) {
    ...
    if(isShow_ == 2){
      shut_img_pub_.publish(cv_bridge::CvImage(img_msg->header, "rgb8", img).toImageMsg());
    }
...
void ArmorDetectorNode::createDebugPublishers()
{
  ...
  shut_img_pub_ = image_transport::create_publisher(this, "/detector/shut_img");
}
