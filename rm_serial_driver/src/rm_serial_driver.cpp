  ...
  serial_driver_{new drivers::serial_driver::SerialDriver(*owned_ctx_)}
{
  ...
  sertial_pub = this->create_publisher<auto_aim_interfaces::msg::OutposeReceive>("/sertial_info_",10);
  ...
  while (rclcpp::ok()) {
    try {
      ...
      if (header[0] == 0x5A) {
        ...
        bool crc_ok =
          crc16::Verify_CRC16_Check_Sum(reinterpret_cast<const uint8_t *>(&packet), 20U/*sizeof(packet)*/);
        if (crc_ok) {
          ...
          serial_data.shot_v = packet.shot_v;
          sertial_pub->publish(serial_data);          
...
void RMSerialDriver::sendData(const auto_aim_interfaces::msg::Target::SharedPtr msg)
{
  ...
  try {
    ...
    packet.shootstatus = msg->shootstatus;
    ...
    packet.dz = msg->dz;
    crc16::Append_CRC16_Check_Sum(reinterpret_cast<uint8_t *>(&packet), 56U /*sizeof(packet)*/);
